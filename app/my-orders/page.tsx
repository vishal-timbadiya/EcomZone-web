...
export default function MyOrdersPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/orders/user`, {
      ...
      .then((res) => {
        if (!res.ok) {
          console.error('Orders response error:', res.status, res.statusText);
      ...
      .then((data) => {
        ...
        if (Array.isArray(data)) {
          console.log('Orders items:', data.map(o => ({ orderId: o.orderId, itemsCount: o.items?.length || 0 })));
          setOrders(data);
          setError(null);
        } else if (data.error) {
          setError(data.error);
