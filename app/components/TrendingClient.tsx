...
export default function TrendingClient() {
  ...
  // Filter states
  const [trendingTab, setTrendingTab] = useState("bestselling");
  ...
  const filteredProducts = useMemo(() => {
    ...
    // Trending tab filter
    if (trendingTab === 'bestselling') {
      result = [...result].sort((a, b) => (b.sales || 0) - (a.sales || 0));
    } else if (trendingTab === 'new-arrival') {
      result = [...result].sort((a, b) => {
        const dateA = new Date(a.createdAt || 0).getTime();
        const dateB = new Date(b.createdAt || 0).getTime();
        return dateB - dateA;
      });
    } else if (trendingTab === 'top-ranking') {
      result = [...result].sort((a, b) => (b.sales || 0) - (a.sales || 0));
    }
    ...
    return result;
  }, [allProducts, stockStatus, selectedCategories, sortBy, trendingTab, minPrice, maxPrice]);
  ...
  const trendingTabs = [
    { id: 'bestselling', label: 'Bestselling', emoji: '🏆' },
    { id: 'new-arrival', label: 'New Arrival', emoji: '✨' },
    { id: 'top-ranking', label: 'Top Ranking', emoji: '⭐' },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-2 sm:px-4 md:px-6 py-2 sm:py-3">
          <nav className="text-xs sm:text-sm">
            <ol className="flex items-center space-x-2">
              ...
              <li className="text-gray-900 font-medium">Trending Products</li>
            </ol>
          </nav>
        </div>
      </div>
      ...
      {/* Gradient Header */}
      <div className="bg-gradient-to-r from-orange-500 via-pink-500 to-red-500 text-white py-4 sm:py-6 md:py-8 mb-4 sm:mb-6 md:mb-8">
        <div className="max-w-7xl mx-auto px-2 sm:px-4 md:px-6 text-center">
          ...
          <p className="text-xs sm:text-base md:text-lg opacity-90">Discover the hottest items everyone is loving</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 md:px-6 py-4 md:py-8">
        ...
        <div className="flex gap-2 sm:gap-3 mb-4 md:mb-6 sticky top-0 bg-gray-50 py-3 md:py-4 z-40 flex-wrap">
          {trendingTabs.map(tab => (
            <button
              ...
              className={`px-3 sm:px-4 md:px-6 py-1 sm:py-2 rounded-full font-medium text-xs sm:text-sm transition whitespace-nowrap ${
                trendingTab === tab.id
        ...
        {/* Stock Status Tabs */}
        <div className="flex gap-2 sm:gap-3 mb-4 md:mb-6 sticky top-12 bg-gray-50 py-3 md:py-4 z-40 flex-wrap">
          <button
        ...
        <div className="flex gap-4 md:gap-8 flex-col md:flex-row">
          ...
          <div className="w-full md:w-56 lg:w-64 flex-shrink-0">
            <div className="bg-white rounded-lg p-4 md:p-6 sticky top-28 md:top-32">
              ...
              <div className="mb-4 md:mb-6 pb-4 md:pb-6 border-b">
                <label className="block text-xs md:text-sm font-semibold text-gray-700 mb-3 md:mb-4">Price Range</label>
                <div className="space-y-2 md:space-y-3">
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      ...
                      className="px-3 py-2 border border-gray-300 rounded-lg text-xs md:text-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
                    />
                    <input
                      type="number"
                      value={maxPrice}
                  ...
                  <div className="flex items-center justify-between gap-2 bg-blue-50 p-2 md:p-3 rounded-lg">
                    <div className="text-center flex-1">
                      <div className="text-sm md:text-lg font-bold text-gray-900">₹{minPrice}</div>
                      <div className="text-xs text-gray-600">Min</div>
                    </div>
                    <span className="text-gray-400">-</span>
                    <div className="text-center flex-1">
                      <div className="text-sm md:text-lg font-bold text-gray-900">₹{maxPrice}</div>
              ...
              {/* Categories */}
              <div>
                <label className="block text-xs md:text-sm font-semibold text-gray-700 mb-2 md:mb-3">Categories</label>
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  ...
                  {categories.map((category) => (
                    <label key={category} className="flex items-center gap-2 text-xs md:text-sm">
                      <input
                        type="checkbox"
              ...
              <button
                ...
                className="w-full mt-4 md:mt-6 bg-orange-600 hover:bg-orange-700 text-white px-3 md:px-4 py-2 rounded-lg font-medium text-xs md:text-sm transition"
              >
                Reset Filters
              </button>
            </div>
          ...
          <div className="flex-1">
            {loading ? (
              ...
              <div className="bg-white rounded-xl shadow-md p-6 sm:p-8 md:p-12 text-center">
                ...
                <button
                  onClick={handleResetFilters}
                  className="inline-block bg-orange-600 hover:bg-orange-700 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-medium text-xs sm:text-sm transition"
                >
                  Reset Filters
                </button>
              </div>
