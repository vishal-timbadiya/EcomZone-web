...
export default function CartPage() {
  ...
  const loadCart = useCallback(async () => {
    ...
    try {
      const productPromises = storedCart
        .filter((item) => {
        ...
        .map(async (item) => {
          ...
          return { id: productId, ...(await res.json()) };
        });
      ...
      storedCart.forEach((item, index) => {
        if (productsData[index]) {
          const productId = item.productId || (item as any).id;
          productsMap[productId] = productsData[index];
        }
      });
  ...
  const handleUpdateQuantity = (productId: string, type: 'single' | 'carton', increment: boolean) => {
    const item = cart.find(i => i.productId === productId || (i as any).id === productId);
    if (!item) return;
