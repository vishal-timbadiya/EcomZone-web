...
export default function CheckoutPage() {
    ...
    const fetchProducts = async () => {
      try {
        ...
        const data = await Promise.all(
          cartItems.map(i => fetch(`${apiUrl}/products/${i.productId}`).then(r => r.ok ? r.json() : null))
        );
  ...
  const handlePlaceOrder = async () => {
    ...
    setTermsError("");
    if (!cartItems.length || finalTotal < MIN_ORDER_VALUE || !validate()) {
      console.log('Validation failed:', { cartItemsLength: cartItems.length, finalTotal, minRequired: MIN_ORDER_VALUE });
      return;
    }
      ...
      <div key={item.productId} className="border p-4 rounded-xl bg-white">
        <div className="flex gap-4">
          <div className="w-20 h-20 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
            {p.imageUrl && <img src={getAbsoluteImageUrl(p.imageUrl)} alt={p.name} className="w-full h-full object-cover"
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2">
                    ...
                    {!codEnabled && !upiEnabled && (
                      <p className="text-red-600 text-sm">No payment methods available. Please contact support.</p>
                    )}
                  </div>
                </div>
