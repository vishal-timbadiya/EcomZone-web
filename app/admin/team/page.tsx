...
export default function AdminTeam() {
  ...
  const fetchSubAdmins = async () => {
    ...
    const res = await fetch(`${apiUrl}/auth/sub-admin`, {
      headers: {
  ...
  const saveEdit = async (id: string) => {
    ...
    const res = await fetch(`${apiUrl}/auth/sub-admin/${id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  const toggleSubAdmin = async (id: string) => {
    ...
    const res = await fetch(`${apiUrl}/auth/sub-admin/${id}/toggle`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 md:p-8 lg:p-10">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-xl sm:text-2xl font-semibold">Admin Team</h1>
        <button
          onClick={() => setShowModal(true)}
          className="bg-black text-white px-3 sm:px-6 py-2 rounded"
        >
          Add Sub-Admin
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {subAdmins.map((admin) => (
            <div key={admin.id} className="bg-white p-4 sm:p-6 rounded-xl shadow">
              {editingId === admin.id ? (
                // Edit Mode
                <div>
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="font-semibold text-base sm:text-lg">Edit Sub-Admin</h3>
                    <button
                      onClick={cancelEdit}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full border px-3 sm:px-4 py-2 sm:py-3 rounded"
                      />
                    </div>
                    <div>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      disabled={submitting}
                      className="bg-green-500 text-white px-3 sm:px-6 py-2 rounded disabled:opacity-50"
                    >
                      {submitting ? "Saving..." : "Save Changes"}
                    </button>
                    <button
                      onClick={cancelEdit}
                      className="border px-3 sm:px-6 py-2 rounded"
                    >
                      Cancel
                    </button>
                  </div>
                ...
                <div>
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="font-semibold text-base sm:text-lg">{admin.name}</h3>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="bg-blue-500 text-white px-2 sm:px-4 py-2 rounded text-sm"
                    >
                      Edit
                    </button>
                    <button
                    ...
                    <button
                      ...
                      className="bg-red-100 text-red-700 py-2 px-2 sm:px-4 rounded text-sm hover:bg-red-200"
                    >
                      Delete
                    </button>
                  </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-4 sm:p-8 w-full max-w-md">
            <h2 className="text-lg sm:text-xl font-semibold mb-6">Add Sub-Admin</h2>
