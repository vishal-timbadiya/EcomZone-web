...
export default function CategoriesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000/api";
      const url = editingId 
        ? `${apiUrl}/admin/categories/update/${editingId}`
  ...
  const handleMove = async (id: string, direction: 'up' | 'down') => {
    try {
      const token = localStorage.getItem("adminToken");
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000/api";
      const res = await fetch(`${apiUrl}/admin/categories/update/${id}`, {
        method: 'POST',
        headers: { 
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
  ...
  const handleToggleActive = async (id: string, isActive: boolean) => {
    ...
    try {
      const token = localStorage.getItem("adminToken");
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000/api";
      const res = await fetch(`${apiUrl}/admin/categories/update/${id}/toggle`, {
        method: 'PATCH',
        headers: { 
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      <div className="max-w-7xl mx-auto p-4 sm:p-6 md:p-8">
        <div className="mb-8">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-800 mb-2">Categories</h1>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 md:gap-6">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2 sm:py-3 border-2 border-slate-200 rounded-xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-200 text-lg"
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2 sm:py-3 border-2 border-slate-200 rounded-xl focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-200 text-2xl"
                placeholder="📦"
                maxLength={2}
              />
            </div>
            ...
            <div className="md:col-span-2">
              <button
                type="submit"
                disabled={saving}
                className="w-full bg-gradient-to-r from-blue-500 to-purple-600 text-white py-3 sm:py-4 px-4 sm:px-8 rounded-2xl font-semibold text-base sm:text-lg shadow-lg hover:shadow-xl hover:scale-[1.02] transform transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
              >
                {saving ? (
                  <>
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          <div className="px-4 sm:px-6 md:px-8 py-4 sm:py-6 border-b border-slate-200">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-xl sm:text-2xl font-bold text-slate-800">All Categories</h2>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-slate-50">
                    <th className="px-3 sm:px-4 md:px-8 py-3 sm:py-4 text-left font-semibold text-slate-700">Icon</th>
                    <th className="px-3 sm:px-4 md:px-8 py-3 sm:py-4 text-left font-semibold text-slate-700">Name</th>
                    <th className="px-3 sm:px-4 md:px-8 py-3 sm:py-4 text-left font-semibold text-slate-700">Slug</th>
                    <th className="px-3 sm:px-4 md:px-8 py-3 sm:py-4 text-left font-semibold text-slate-700">Status</th>
                    <th className="px-3 sm:px-4 md:px-8 py-3 sm:py-4 text-left font-semibold text-slate-700">Created</th>
                    <th className="w-48 px-3 sm:px-4 md:px-8 py-3 sm:py-4 text-right font-semibold text-slate-700">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {categories.map((category) => (
                    <tr key={category.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                      <td className="px-3 sm:px-4 md:px-8 py-4 sm:py-6">
