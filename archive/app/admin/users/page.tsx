      ...
      <div className="bg-white p-4 rounded-xl shadow mb-6">
        <div className="flex flex-wrap gap-4 items-center">
          <div className="flex-1 min-w-[200px]">
            <input type="text" placeholder="Search..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full border p-2 rounded" />
          </div>
          ...
          <div className="min-w-[150px]">
            <select value={sortField} onChange={(e) => setSortField(e.target.value as SortField)} className="w-full border p-2 rounded">
          ...
          <div className="min-w-[100px]">
            <select value={sortOrder} onChange={(e) => setSortOrder(e.target.value as SortOrder)} className="w-full border p-2 rounded">
                ...
                <tr key={user.id} className="border-b">
                  {editingId === user.id ? (
                    <>
                      ...
                      <td className="py-2"><input type="text" value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} className="border p-1 w-32" /></td>
                      <td className="py-2"><input type="email" value={editForm.email} onChange={(e) => setEditForm({ ...editForm, email: e.target.value })} className="border p-1 w-40" /></td>
                      <td className="py-2"><input type="tel" value={editForm.mobile} onChange={(e) => setEditForm({ ...editForm, mobile: e.target.value })} className="border p-1 w-28" /></td>
                      ...
                      <td className="py-2">
                        <button onClick={() => saveEdit(user.id)} className="bg-green-500 text-white px-2 py-1 rounded text-xs mr-1">Save</button>
                    ...
                    <>
                      ...
                      <td className="py-2">{new Date(user.createdAt).toLocaleDateString()}</td>
                      <td className="py-2">
                        <button onClick={() => setResetPasswordId(user.id)} className="bg-orange-500 text-white px-2 py-1 rounded text-xs mr-1">Reset</button>
                        <button onClick={() => startEdit(user)} className="bg-blue-500 text-white px-2 py-1 rounded text-xs mr-1">Edit</button>
                        <button onClick={() => deleteUser(user.id)} className="bg-red-500 text-white px-2 py-1 rounded text-xs">Delete</button>
                      </td>
                    </>
                  )}
                </tr>
