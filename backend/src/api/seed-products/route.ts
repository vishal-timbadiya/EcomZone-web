...
export async function POST() {
  try {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const variant = i >= productData.length ? ` V${Math.floor(i / productData.length) + 1}` : "";
      
      const singlePrice = data.basePrice + Math.floor(Math.random() * 20) - 10;
      const cartonPrice = Math.floor(singlePrice * data.ctnQty * 0.85);
      const cartonPcsPrice = cartonPrice / data.ctnQty;

      products.push({
        name: data.name + variant,
        slug: data.name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") + "-" + i,
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ 
      success: true, 
      count,
      message: `${count} products in database`
    });
  } catch (error) {
    return NextResponse.json(
